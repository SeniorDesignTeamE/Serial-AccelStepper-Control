...
//pull high PUL+,DIR+,ENA+ to arduinos 5V
...
const int STEPPER_4_STEP_PIN = 11;
const int STEPPER_4_DIR_PIN = 12;
const int STEPPER_4_ENABLE_PIN = 13;
...
const byte STEP_COMMAND_STOP = 0x0D;
const unsigned int STEP_COMMAND_STOP_LENGTH = 6;
    ...
    { 
      ...
      if(count >= packetSize)
      {
        ...
        if(validatePacket(packetSize, buffer))
        {
          ...
          else if(packetSize == STEP_COMMAND_STOP_LENGTH)
          {
            Stepper_1.stop();
            Stepper_2.stop();
          }
